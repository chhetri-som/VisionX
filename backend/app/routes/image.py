...
from app.schemas.responses import AnalyzeResponse, ChatInferenceResponse, ChatMessage, FaceResult, ForensicsResponse
...
from app.dependencies import get_face_detector, get_image_classifier, get_findings_engine
...
async def analyze_image(
    ...
    image_classifier=Depends(get_image_classifier),
    findings_engine=Depends(get_findings_engine)
):
    ...
    try:
        ...
        for idx, face in enumerate(detected_faces):
            ...
            confidence, reasoning = image_classifier.classify(face_crop_resized)

            # Findings + structural metrics
            label, findings, landmarker_data = findings_engine.generate_findings(confidence, face)
...
@router.post("/image/chat", response_model=ChatInferenceResponse)
async def chat_with_image(
    prompt: str = Form(..., description="The user's specific question"),
    forensic_thoughts: str = Form(..., description="Everything generated by VLM under <think> tags, used as context."),
    history: str = Form("[]", description="JSON string of previous ChatMessage objects"),
    image_classifier = Depends(get_image_classifier)
):
    start_time = time.time()
    try:
        ...
        try:
            ...
            validated_history = [ChatMessage(**msg).model_dump() for msg in raw_history]
        except json.JSONDecodeError:
            raise HTTPException(status_code=400, detail="History must be a valid JSON string.")
        except ValidationError as e:
            # returns which exact part of schema is invalid
            raise HTTPException(status_code=422, detail=f"History schema validation failed: {e.errors()}")
        logger.info(
            f"/image/chat prompt_length={len(prompt)} "
            f"history_length={len(validated_history)}"
        )
        # model check
        if image_classifier is None:
            raise HTTPException(status_code=503, detail="VLM is not loaded.")
        
        # execute inference
        reply = image_classifier.chat_inference(
            user_prompt=prompt,
            forensic_thoughts=forensic_thoughts,
            history=validated_history
        )

        execution_time = int((time.time() - start_time) * 1000)
        logger.info(f"/image/chat success execution_time_ms={execution_time}")
        ...
        logger.error(f"Unexpected error in /image/chat: {e}", exc_info=True)
...
async def analyze_forensics(
    ...
    try:
        ...
        if run_full_pipeline:
            ...
            # use per-face CFA analysis when auto-detected bbox is available
            if bbox:
                ...
                # fallback to whole-image analysis if no face bbox (should not reach here
                # inside run_full_pipeline, but kept as defensive fallback)
            ...
            fft_result = FFTAnalyzer.analyze(image_bytes, bbox=bbox)
